import React from 'react';
import './style.scss';

const Apropos = () => {
  
  return (
    <div id="lienapropos" className="apropos">
      <h2 className="apropos-title">A Propos de moi</h2>
      <p className="apropos-paragraph">Né en 1984, père de deux enfants, marié, passionné de sport. j’ai effectué 
ma reconversion en 2021 en participant à la promotion O’clock Marty de 
septembre à mars 2021. J'ai obtenu mon titre pro Développeur Web & Web mobile en juin 2021.</p>
      <p className="apropos-paragraph">Vous découvrirez ici quelques uns de mes projets personnels et 
professionnels déjà effectuées.</p>
    </div>
  );
}

export default Apropos;
